...
import { CaseType, WorkflowStep } from "@/types";
import { getCaseById } from "@/supabase/cases";
...
export default async function CaseDetailPage({
  ...
  const currentStep: WorkflowStep | null = currentStepDef
    ? {
        id: `${caseRecord.id}-step-${currentStepIndex + 1}`,
        case_id: caseRecord.id,
        step_name: currentStepDef.name,
        department: currentStepDef.department,
        step_order: currentStepIndex + 1,
        status: "in_progress",
        assigned_officer_id: null,
        estimated_processing_days: currentStepDef.estimated_processing_days,
        required_skill: currentStepDef.required_skill,
        required_authority: currentStepDef.required_authority,
        // No live queue-length signal per case yet — use the recommended
        // officer's own current_load as a deterministic proxy rather than
        // a hardcoded demo number.
        queue_length: 0,
      }
    : null;

  const recommendation = currentStep ? recommendOfficer(officers, currentStep) : null;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="gf-card overflow-hidden">
          <div className="px-6 py-4 border-b" style={{ borderColor: "var(--border)" }}>
            ...
            <p className="text-xs mt-1" style={{ color: "var(--muted)" }}>
              Step {currentStepIndex + 1} of {template.length || "—"}.
            </p>
          </div>

          <div className="px-6 py-4 space-y-3 text-sm">
            {template.length === 0 && (
              <p style={{ color: "var(--muted)" }}>No workflow template defined for this case type.</p>
            )}
            {template.map((step, i) => (
              <div key={step.name} className="flex items-center justify-between">
                <span style={{ color: i === currentStepIndex ? "var(--navy)" : "var(--muted)" }} className={i === currentStepIndex ? "font-semibold" : ""}>
                  {i + 1}. {step.name}
                </span>
                <span className="text-xs" style={{ color: "var(--muted)" }}>{step.department}</span>
              </div>
            ))}
          </div>
        </div>
